...
function Home() {
  ...
  const getListRec = async (startDate, endDate) => {
    try {
      const response = await axios.get(`api/recap/data/${startDate}/${endDate}`);
  ...
  const getTotalAmount = (hotel, date) => {
    const ordersOnDate = data.filter(d => d.nm_hotel === hotel && d.order_date === date);
  ...
  const renderTableData = () => {
    ...
    return hotelNames.map((hotel, index) => {
      ...
      const totalPrice = totalAmountForHotel * parseFloat(price.replace('.', '').replace(',', '.'));
      return (
        <tr key={index}>
          <td>{hotel}</td>
          {range.map((date, i) => (
  ...
  const renderTotalRow = () => {
    ...
    const priceTotalTotal = totalAll * parseFloat(price.replace('.', '').replace(',', '.'));
  ...
  return (
    <div>
      <Navbar bg="dark" variant="dark" expand="lg" className='mb-5'>
        <Container>
          <Navbar.Brand as={Link} to="/">Meals Order</Navbar.Brand>
      ...
      <Container className="d-flex justify-content-center align-items-center" style={{ height: 'auto' }}>
        <Form className="d-flex flex-row" onSubmit={handleSearch}>
          ...
          <Button variant="primary" type="submit" className="align-self-end mb-3 me-2" style={{ fontSize: '11px' }}>
            Search
          </Button>
        </Form>
      </Container>
      ...
      <Container className="d-flex justify-content-center align-items-center" style={{ height: 'auto' }}>
        <Card className="text-justify" style={{ width: '100%', fontSize: '11px' }}>
          <Card.Body>
            <Card.Title>REKAPITULASI CATERING</Card.Title>
            <Card.Text>
              Set date range to get report
            </Card.Text>
            <Card.Text>
              Entered Price : Rp{price}
            </Card.Text>
            ...
            <Button variant="primary" type="submit" className="align-right me-2" style={{ fontSize: '11px' }} onClick={onDownload}>
              Download Excel
            </Button>
          </Card.Body>
